...
module.exports.onCommand = (client, msg) => {
  fu.getDB(msg.member.id, (result) => {
    ...
    msg.channel.send('돈을 선택해 걸어주세요.\n:one:: 100원\n:two:: 1000원\n:three:: 5000원\n:four:: 10000원').then((mesg) => {
        ...
        let collector1 = mesg.createReactionCollector(f1, { time: 60000 }) //빈응 컬렉터 생성

        let f2 = (reaction, user) => reaction.emoji.name === "2️⃣" && user.id === msg.author.id
        let collector2 = mesg.createReactionCollector(f2, { time: 60000 }) //빈응 컬렉터 생성

        let f3 = (reaction, user) => reaction.emoji.name === "3️⃣" && user.id === msg.author.id
        let collector3 = mesg.createReactionCollector(f3, { time: 60000 }) //빈응 컬렉터 생성

        let f4 = (reaction, user) => reaction.emoji.name === "4️⃣" && user.id === msg.author.id
        let collector4 = mesg.createReactionCollector(f4, { time: 60000 }) //빈응 컬렉터 생성
